...
class AcademyPage {
  ...
  getSocialMetas() {
    ...
    return updateStaticAssetLinks(
      metas.map((element) => this.$.html(element)).join("")
    );
  }
  ...
  getCSSLinks() {
    const cssLinks = this.$("link")
      ...
      .map(
        (element) =>
          `<link rel="stylesheet" type="text/css" href="${element.attribs.href}">`
      )
      .join();
...
const updateStaticAssetLinks = (html) => {
  ...
  let processedHtml = html.replace(
    /<script[^>]*id=["'`]dynamic-card-generator["'`][^>]*>([\s\S]*?)<\/script>/g,
    (match, scriptContent) => {
